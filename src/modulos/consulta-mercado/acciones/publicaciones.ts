...
export async function consultarPublicacionesAgrupadas(): Promise<resultadoPublicacionesAgrupadas> {
    const consulta = db.sql.public.publicacionOperador
        .innerJoin(db.sql.public.publicacion, (tablas, operaciones) =>
            operaciones.eq(tablas.publicacionOperador.publicacionId, tablas.publicacion.id))
        .innerJoin(db.sql.public.presentacion, (tablas, operaciones) =>
    ...
    const publicacionesPorOperador = new Map<number, operadorListado>();

    for (const fila of filas) {
        const operadorId = Number(fila.operadorId);

        let operador = publicacionesPorOperador.get(operadorId);
    ...
    return { operadores: Array.from(publicacionesPorOperador.values()) };
}

// DE ACA PARA ABAJO ES SI SE TERMINA HACIENDO EN LOTES
...
// export async function consultarPublicacionesEnLotes(filtros: filtrosPublicaciones = {}): Promise<resultadoPublicaciones> {
...
// /* 
// export async function consultarPublicacionesAgrupadasEnLotes(filtros: filtrosPublicaciones = {}): Promise<resultadoPublicacionesAgrupadas> {
//     // Se arma la consulta base
//     let consulta = db.sql.public.publicacionOperador
//         .innerJoin(db.sql.public.publicacion, (tablas, operaciones) =>
//                 operaciones.eq(tablas.publicacionOperador.publicacionId, tablas.publicacion.id))
