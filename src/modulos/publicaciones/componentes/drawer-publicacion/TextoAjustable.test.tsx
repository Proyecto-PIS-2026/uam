...
class ObservadorDePrueba implements ResizeObserver {
  ...
  constructor(private callback: ResizeObserverCallback) {
    ObservadorDePrueba.instancias.push(this);
  }

  cambiarAncho(ancho: number) {
    this.callback(
      [{ contentRect: new DOMRect(0, 0, ancho, 20) } as ResizeObserverEntry],
      this,
    );
  }
}
...
// JSDOM no calcula dimensiones ni carga fuentes: se simulan esas APIs del navegador.
beforeEach(() => {
  anchoContenedor = 202;
  factorFuente = 1;
  ...
  fuentesListas = new Promise<void>((resolve) => {
    resolverFuentes = resolve;
  });
...
describe("TextoAjustable", () => {
  it("muestra el texto completo en el título y aplica la clase recibida", () => {
    render(
      <TextoAjustable
        texto="Tomate"
        minimo={10}
    ...
    expect(screen.getByTitle("Tomate")).toHaveClass("especie");
  });

  it("mantiene el tamaño máximo cuando el texto cabe", () => {
    render(<TextoAjustable texto="Tomate" minimo={10} maximo={30} />);
  ...
  it.each([10, 10.3])(
    ...
    (minimo) => {
      anchoContenedor = 12;
      render(<TextoAjustable texto="Tomate" minimo={minimo} maximo={30} />);

      expect(screen.getByText("Tomate")).toHaveStyle({
        fontSize: `${minimo}px`,
      });
    },
  );
  ...
  it("reduce y vuelve a ampliar el texto cuando cambia el ancho", () => {
    ...
    expect(observador.observe).toHaveBeenCalledExactlyOnceWith(
      screen.getByTitle("Tomate"),
    );
  ...
  it("recalcula al cambiar el texto y los límites de tamaño", () => {
    ...
    const { rerender } = render(
      <TextoAjustable texto="Tomate" minimo={10} maximo={30} />,
    );

    rerender(<TextoAjustable texto="Uva" minimo={10} maximo={30} />);
    expect(screen.queryByText("Tomate")).not.toBeInTheDocument();
    expect(screen.getByText("Uva")).toHaveStyle({ fontSize: "30px" });

    rerender(<TextoAjustable texto="Uva" minimo={10} maximo={20} />);
    expect(screen.getByText("Uva")).toHaveStyle({ fontSize: "20px" });

    anchoContenedor = 12;
    rerender(<TextoAjustable texto="Uva" minimo={12} maximo={20} />);
    expect(screen.getByText("Uva")).toHaveStyle({ fontSize: "12px" });
    expect(ObservadorDePrueba.instancias[0].disconnect).toHaveBeenCalledTimes(
      1,
    );
  });
  ...
  it("reajusta el tamaño al terminar de cargar nuevas fuentes", () => {
    ...
    act(() => {
      fuentes.dispatchEvent(new Event("loadingdone"));
    });
  ...
  it("desconecta el observer y deja de ajustar el texto después de desmontarse", async () => {
    ...
    expect(quitarListener).toHaveBeenCalledExactlyOnceWith(
      "loadingdone",
      listener,
    );
    vi.mocked(HTMLSpanElement.prototype.getBoundingClientRect).mockClear();
