...
describe("TarjetaPublicacionConOperador", () => {
    it("muestra el producto, sus características y el operador", () => {
        render(<TarjetaPublicacionConOperador publicacion={crearPublicacion()} onClick={vi.fn()}/>);
        ...
        const operadores = screen.getAllByText("Huerta Sur");
        expect(operadores).toHaveLength(2);
        expect(operadores[0]).toHaveAttribute("title", "Huerta Sur");
        expect(operadores[1]).toHaveAttribute("title", "Huerta Sur");
        expect(screen.getByRole("button", { name: "Ver detalles de Tomate - Perita" })).toHaveAttribute("type", "button");
    });

    it.each(["-", ""])(
        "muestra solamente la especie cuando la variedad es %j",
        (variedad) => {
            render(<TarjetaPublicacionConOperador publicacion={{ ...crearPublicacion(), variedad }} onClick={vi.fn()}/>);
            expect(screen.getByRole("heading", { name: "Tomate" })).toBeInTheDocument();
            expect(screen.getByRole("button", { name: "Ver detalles de Tomate" })).toBeInTheDocument();
        }
    );

    it("muestra la foto con un texto alternativo del producto", () => {
        render(<TarjetaPublicacionConOperador publicacion={{ ...crearPublicacion(), foto: "/tomate.jpg" }} onClick={vi.fn()}/>);
    ...
    it.each([
        ...
        "muestra el precio %s en pesos uruguayos",
        (precio, esperado) => {
            render(<TarjetaPublicacionConOperador publicacion={{ ...crearPublicacion(), precio }} onClick={vi.fn()}/>);
    ...
    it("ejecuta el callback al seleccionar la tarjeta", () => {
        const onClick = vi.fn();
        render(<TarjetaPublicacionConOperador publicacion={crearPublicacion()} onClick={onClick}/>);
	...
	it("muestra consultar precio cuando el precio es null", () => {
		render(<TarjetaPublicacionConOperador publicacion={{ ...crearPublicacion(), precio: null }} onClick={vi.fn()}/>);
